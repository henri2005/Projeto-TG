...
import 'package:turisr/pages/convidado/estabelecimento_screen.dart';

class EstabelecimentoConvidado extends StatefulWidget {
  const EstabelecimentoConvidado({super.key, required this.title});

  final String title;

  @override
  State<EstabelecimentoConvidado> createState() => _MyHomePageState();
}

class _MyHomePageState extends State<EstabelecimentoConvidado> {
  ...
  void carregarEstabelecimentos() async {
    try {
      Loading.show(context, mensagem: 'Carregando estabelecimentos...');
      ...
      String repository = 'http://10.0.0.94/api_turismo/locais/tipo/E';
  ...
  void listarPesquisas(String consulta) async {
    if (consulta == "") {
      itensListados = listaEstabelecimentos;
    } else {
      itensListados =
          listaEstabelecimentos
              .where(
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      carregarEstabelecimentos(); // só executa a função após carregar a árvore de widgets
    });
  }
                                      ...
                                      onPressed: () async {
                                        Navigator.push(
                                          ...
                                          MaterialPageRoute(
                                            builder:
                                                (context) =>
                                                    EstabelecimentoScreenConvidado(
